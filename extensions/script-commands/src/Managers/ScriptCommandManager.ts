...
export class ScriptCommandManager {
  ...
  finishSetup(scriptCommand: ScriptCommand): StateResult {
    ...
    return {
      content: State.Error,
      message:
        "Something went wrong to confirm the setup for the Script Command",
    }
  }
  ...
  private async downloadIcons(
    ...
    icons.light = await this.downloadIconFor(
      scriptCommand,
      lightIcon,
      imageFolderPath,
      IconStyle.Light,
  ...
  private async downloadIconFor(
    ...
    resource =
      style == IconStyle.Light
        ? iconLightURLFor(scriptCommand)
        : iconDarkURLFor(scriptCommand)
  ...
  private async downloadCommand(
    ...
    const linkFilename = scriptCommand.isTemplate
      ? `${scriptCommand.identifier}.template`
    ...
    if (!existsSync(commandFilePath)) {
      await download(sourceCodeRawURL(scriptCommand), commandPath, {
        filename: filename,
      })
    }
